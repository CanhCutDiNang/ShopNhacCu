                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                            ...
                            if (saobandau != 0.0) {
                                ...
                                databaseReference.child("SanPham").child(key).child("BinhLuan").child(uid).setValue(binhLuan);
                                databaseReference.child("SanPham").child(key).child("soLike").setValue(tongsao1).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                ...
                                databaseReference.child("SanPham").child(key).child("BinhLuan").child(uid).setValue(binhLuan);
                                databaseReference.child("SanPham").child(key).child("soLike").setValue(sosaosau).addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        Fragment fragment = new DonHangCuaToiFragment();
                                        FragmentManager fmgr = getActivity().getSupportFragmentManager();
